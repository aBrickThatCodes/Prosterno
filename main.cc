...
void usage(const std::string_view& s) {
  ...
  std::cout << "path - path to a scoreboard file, in none entered defaults to "
               "\"scoreboard.txt\""
            << std::endl;
  std::cout << "-h, --help - show this information\n";
  std::cout << "Rules for the game (in Polish) can be found at ";
  std::cout << "https://www.gamesfanatic.pl/2014/04/03/"
               "octonovem-polski-warcabowy-gipf/\n";
  exit(EXIT_SUCCESS);
}
...
int main(int argc, char** argv) {
  ...
  while (true) {
    Game g;
    g.loop();
    std::cout << (g.get_winner() == Game::Player::kWhite ? "White" : "Black")
              << " won!\n";
    switch (g.get_winner()) {
      case Game::kNone:
        break;
